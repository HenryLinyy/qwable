...
def test_alias_model_names_map_to_fusion_agent():
    for p in ("budget", "quality", "coding", "heavy"):
        assert resolve_profile(f"qwable-fusion-{p}", "openai_responses") == "fusion-agent"
        assert resolve_profile(f"qwable-fusion-{p}", "openai_chat") == "fusion-agent"
        assert resolve_profile(f"claude-qwable-fusion-{p}", "anthropic_messages") == "fusion-agent"
...
def test_explicit_fusion_block_wins_over_model_name():
    r = extract_fusion_request({"model": "qwable-fusion-budget", "fusion": {"preset": "quality"}})
    assert r.preset == "quality"
