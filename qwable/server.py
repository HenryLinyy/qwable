...
from qwable.render_anthropic_messages import render_final_answer as render_anthropic_final
...
MODELS_LIST = {
    ...
    "data": [
        ...
        {"id": "claude-qwable-fusion-budget", "object": "model", "owned_by": "local", "type": "model", "display_name": "Claude Qwable Fusion (budget)"},
        {"id": "claude-qwable-fusion-quality", "object": "model", "owned_by": "local", "type": "model", "display_name": "Claude Qwable Fusion (quality)"},
        {"id": "claude-qwable-fusion-coding", "object": "model", "owned_by": "local", "type": "model", "display_name": "Claude Qwable Fusion (coding)"},
        {"id": "claude-qwable-fusion-heavy", "object": "model", "owned_by": "local", "type": "model", "display_name": "Claude Qwable Fusion (heavy)"},
        {
...
async def v1_fusion_presets():
    ...
    try:
        ...
        for line in out.splitlines():
            parts = line.split()
            if len(parts) >= 2 and "/" in parts[0] and parts[0] not in (
                "IDENTIFIER", "No", "To",
            ):
    ...
    for name, p in PRESETS.items():
        ds4_model = getattr(cfg, "ds4_model", "deepseek-v4-flash") if cfg else "deepseek-v4-flash"
        presets_dict[name] = {
    ...
    return {
        ...
        "default_preset": getattr(cfg, "fusion_default_preset", "quality") if cfg else "quality",
    }
...
def _build_models_health() -> dict:
    ...
    for stage in (
        ...
        out["roles"].setdefault(
            role,
            {"primary": sel.model_name, "fallbacks": list(sel.fallback_chain), "stages": []},
        )
...
async def _stream_chat_response(body: dict, task):
    ...
    if task.profile == "fusion-agent":
        ...
        try:
            preset = resolve_preset(fusion_req, default=fusion_core.config.fusion_default_preset)
        ...
        try:
            yield sse_event(": fusion preset resolved: " + preset.name + "\n\n", event=None)
            streamed_text = ""
            async for ev in _events_with_keepalive(run_fusion_agent_streaming(
    ...
    if task.profile == "fusion-agent":
        ...
        try:
            ...
            try:
                ...
                yield sse_event(
                    {"type": "response.in_progress", "response": {"id": response_id, "model": model}},
    ...
    if task.profile == "fusion-agent":
        ...
        try:
            ...
            try:
                ...
                yield sse_event({"type": "content_block_start", "index": 0, "content_block": {"type": "text", "text": ""}}, event="content_block_start")
                yield sse_event({"type": "content_block_delta", "index": 0, "delta": {"type": "text_delta", "text": action.text or ""}}, event="content_block_delta")
                yield sse_event({"type": "content_block_stop", "index": 0}, event="content_block_stop")
                yield sse_event({"type": "message_delta", "delta": {"stop_reason": "end_turn", "stop_sequence": None}}, event="message_delta")
        ...
        try:
            ...
            yield sse_event(
                {"type": "message_delta",
                 ...
                 "usage": {"output_tokens": estimate_tokens(streamed_text) if streamed_text else 0}},
                event="message_delta",
            )
            yield sse_event({"type": "message_stop"}, event="message_stop")
        finally:
